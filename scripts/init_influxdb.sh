...
if [[ -z $(influx -host $INFLUX_HOST -port $INFLUX_PORT -username $INFLUXDB_USER -password $INFLUXDB_USER_PASSWORD -execute "SHOW MEASUREMENTS ON $DATABASE_NAME" | grep $TABLE_NAME) ]]; then
